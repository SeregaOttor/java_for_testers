...
public class DeveloperMailHelper extends HelperBase{
    ...
    public DeveloperMailUser addUser() {
        ...
        try (Response response = client.newCall(request).execute()) {
            ...
            var addUserRespose = new ObjectMapper().readValue(text, AddUserResponse.class);
            if (!addUserRespose.success()) {
                throw new RuntimeException(addUserRespose.errors().toString());
            }
            return addUserRespose.result();
        } catch (IOException e) {
